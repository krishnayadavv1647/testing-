    ...
    <>
      ...
      <div className="grid gap-6 xl:grid-cols-[420px_1fr]">
        <form className="card space-y-4" onSubmit={submit}>
          ...
          <p className="text-xs text-slate-500">Use E.164 format, for example +910000000000.</p>
        ...
        <section className="card">
          <h2 className="mb-3 font-bold text-ink">Dograh response</h2>
          {result ? (
            <pre className="max-h-[520px] overflow-auto rounded-lg bg-slate-950 p-4 text-xs text-slate-100">{JSON.stringify(result, null, 2)}</pre>
          ) : (
            <p className="text-sm text-slate-500">The Dograh response will appear after a call is triggered.</p>
          )}
        </section>
      </div>
    </>
